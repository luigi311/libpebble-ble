...
pub fn parse_app_run_state(payload: &[u8]) -> Option<(AppRunStateCmd, String)> {
    let cmd = AppRunStateCmd::from_u8(*payload.first()?)?;
    let uuid_bytes: [u8; 16] = payload.get(1..17)?.try_into().ok()?;
...
mod tests {
    ...
    #[test]
    fn app_run_state_rejects_short() {
        assert!(parse_app_run_state(&[0x01, 0x00]).is_none());
